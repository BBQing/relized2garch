...
.real2garchfit = function(spec, data, out.sample = 0, solver = "solnp", solver.control = list(),
		...
		realizedVol = NULL)
{
	...
	if(is.null(fit.control$scale)){
		...
		if(fit.control$scale){
			warning("\nscaling not valid for real2GARCH model.")
			fit.control$scale=0
		}
	}
	...
	if(is.null(realizedVol)){
	...
	} else{for(i in names(realizedVol)){ ###c pokud je vkladana realizedVol jako xts, tak je for v testu zbytecny
	  rvn = i
	  if(!is(subset(realizedVol, select = i), "xts")) stop(paste("\nugarchfit-->error:", rvn,  " must be an xts object\n", sep = "", collapse = " "))
	  realizedVolIndex = format(index(subset(realizedVol, select = i)), format="%Y-%m-%d")
	  ...
	  if(is.na(match(i,c('RSVp','RSVm')))) stop(paste("\n", rvn ,"not allowed realized volatility measure", sep = "", collapse = " "))
	}
	 }
	...
	if(any(ipars[,2]==1)){
		if(npars == 0){
			if(fit.control$fixed.se==0) {
				...
				return(ugarchfilter(data = xts(origdata, origindex), spec = spec, out.sample = out.sample, realizedVol = realizedVol)) ###tady budu muset intervenovat
			} else{
	...
	if(use.solver){
		...
		solution = .garchsolver(solver, pars = ipars[estidx, 1], fun = .real2garchLLH,
		...
		if(!is.null(sol$par)){
			...
			if(modelinc[7]==0){
				...
				tmpx = .real2garchLLH(sol$par, arglist)
	...
	if(convergence == 0){
		...
		arglist$data = data
		fit = .makefitmodel(garchmodel = "real2GARCH", f = .real2garchLLH, T = T, m = m,
...
.real2garchLLH = function(pars, arglist)
{
  # prepare inputs
  data = arglist$data
  realized = arglist$realized
  ...
  rx = .arfimaxfilter(modelinc[1:21], pars = ipars[,1], idx = idx, mexdata = mexdata, h = hm, data = data, N = N)
  res = rx$res
  #cat("toto je return",head(rx$res,2),sep = "\n",file = "pnames.txt", append = FALSE) ###c .arfimaxfilter funguje
  zrf = rx$zrf
  res[is.na(res) | !is.finite(res) | is.nan(res)] = 0
  mvar = ifelse(recinit$type==1, mean(res[1:recinit$n]*res[1:recinit$n]), backcastv(res, T, recinit$n))
  if(modelinc[15]>0) {
  ...
  hEst = mvar
  # variance targeting
  persist = .persistreal2garch1(ipars, idx, distribution)
  if(modelinc[7]==0){
    mvar2 = ifelse(!is.na(modelinc[22]), modelinc[22]/dscale, mvar)
    ipars[idx["omega",1],1] = log(mvar2) * max(1 - persist, 0.001) - mv
    assign("omega", ipars[idx["omega",1],1], garchenv)
  }
  #cat("toto je persistence",persist,sep = "\n",file = "pnames.txt", append = FALSE) ###c persist funguje
  # E[eres^2]=1
  if(is.na(hEst) | !is.finite(hEst) | is.nan(hEst)) hEst = var(data)
  # if we have external regressors in variance equation we cannot have
  # stationarity checks in likelihood. Stationarity conditions not valid for
  ...
  if(modelinc[15]>0) vexdata = as.double(as.vector(vexdata)) else vexdata = double(1)
  # modelinc (1:21) since 22 is either NA or numeric and no longer needed (paased to ipars if used)
  ans = try( .C("real2garchfilterC", model = as.integer(modelinc[1:35]),
                pars = as.double(ipars[,1]), idx = as.integer(idx[,1]-1),
                hEst = as.double(hEst), x = as.double(data), res = as.double(res),
                mexdata = mexdata, vexdata = vexdata,
                zrf = as.double(zrf), constm = double(T), condm = double(T),
                m = as.integer(m), T = as.integer(T), h = double(T),
                z = double(T),
                taup = double(T),taum = double(T),
                rsvp = as.double(realized$RSVp)[1:T], rsvm = as.double(realized$RSVm)[1:T],
                up = double(T),um = double(T),
                llh = double(1), LHT1P = double(T),
                LHT = double(T),
                PACKAGE = "stealrugarch"), silent = TRUE )
  if(inherits(ans, "try-error")){
    if(arglist$pmode!=1){
      assign("rugarch_csol", 1, envir = garchenv)
      assign("rugarch_filtermessage", ans, envir = garchenv)
  ...
  ans = switch(returnType,
               llh = llh,
               LHT = LHT,
               all = list(llh = llh, h = h, epsx = epsx, z = z, LHT = LHT, persistence = persist,
                          up = up, um = um , taup = taup, taum = taum,
                          LHT1P = -LHT1P))
  return(ans)
}
...
.real2garchfilter = function(spec, data, out.sample = 0, n.old = NULL, rec.init = 'all', realizedVol)
{
  # n.old is optional and indicates the length of the original dataseries (in
  # cases when this represents a dataseries augmented by newer data). The reason
  # for using this is so that the old and new datasets agree since the original
  ...
  #tady je potreba neco udelat s tou kontrolou vstupu

  if(is.null(realizedVol) | missing(realizedVol) ){
    stop("\nugarchfit-->error: you must supply the realized volatility (realizedVol) for the realGARCH model\n")
  } else{for(i in names(realizedVol)){
    rvn = i
    if(!is(realizedVol$i, "xts")) stop(paste("\nugarchfit-->error:", rvn,  "must be an xts object\n", sep = "", collapse = " "))
    realizedVolIndex$i = format(index(realizedVol$i), format="%Y-%m-%d")
    if(!all.equal(realizedVolIndex$i, format(xdata$index, format="%Y-%m-%d"))) stop("\nugarchfit-->error: realizedVol must match the time index of the data")
    if(is.na(match(i,c('RSVp','RSVm')))) stop(paste("\n", rvn ,"not allowed realized volatility measure", sep = "", collapse = " "))
  }
  }
  realized = coredata(realizedVol)
...
.real2garchforecast = function(fitORspec, data = NULL, n.ahead = 10, n.roll = 0, out.sample = 0,
                              external.forecasts = list(mregfor = NULL, vregfor = NULL), n.sim = 1000, returnDistribution = TRUE,
                              ...)
{
  # will not allow to combine n.ahead and n.roll in this model
  ...
  data   = fit@model$modeldata$data
  #realized = coredata(fit@model$modeldata$realizedVol)[,1]
  realized = lapply(fit@model$modeldata$realizedVol, function(x) coredata(x[,1]))
  Nor    = length(as.numeric(data))
  index  = fit@model$modeldata$index
  period = fit@model$modeldata$period
  ns = fit@model$n.start
  ...
  # filter data (check external regressor data - must equal length of origData)
  fcreq = ifelse(ns >= (n.ahead+n.roll), n.ahead+n.roll, ns)
  fspec = myugarchspec(variance.model = list(model = "real2GARCH",
                                           garchOrder = c(modelinc[23], modelinc[9]), submodel = NULL,
                                           external.regressors = vxf[1:(N + fcreq), , drop = FALSE]),
                     mean.model = list(armaOrder = c(modelinc[2], modelinc[3]),
                                       include.mean = modelinc[1],
                                       archm = ifelse(modelinc[5]>0,TRUE,FALSE), archpow = modelinc[5], arfima = modelinc[4],
  ...
  tmp =  xts(data[1:(N + fcreq)], index[1:(N + fcreq)])
  realtmp = lapply(realized, function(x) xts(x[1:(N + fcreq)], index[1:(N + fcreq)]))
  flt = .real2garchfilter(data = tmp, spec = fspec, n.old = N, realizedVol = realtmp)
  sigmafilter = flt@filter$sigma
  resfilter = flt@filter$residuals
  zfilter = flt@filter$z
  upfilter = flt@filter$up
  umfilter = flt@filter$um
  taupfilter = flt@filter$taup
  taumfilter = flt@filter$taum
  seriesfilter = as.numeric(fitted(flt))
  # forecast GARCH process
  realizedRSVpFor = realizedRSVmFor = seriesFor = sigmaFor = matrix(NA, ncol = n.roll+1, nrow = n.ahead)
  colnames(realizedRSVpFor) = colnames(realizedRSVmFor) = colnames(seriesFor) = colnames(sigmaFor) = as.character(index[N:(N+n.roll)])
  rownames(realizedRSVpFor) = rownames(realizedRSVmFor) = rownames(sigmaFor) = paste("T+", 1:n.ahead, sep="")
  if(returnDistribution){ #tady na tohle se jeste udu muset podivat, co to dela
    realizedDpF = realizedDmF = sigmaDF = array(NA, dim = c(n.ahead, n.sim, n.roll+1),
                                 dimnames=list(paste("T+", 1:n.ahead, sep=""), paste("Sim_", 1:n.sim, sep=""), as.character(index[N:(N+n.roll)])))
  }
  for(i in 1:(n.roll+1)){
    np = N + i - 1
    ...
    omega = rep(ipars[idx["omega",1]:idx["omega",2], 1], N+i+n.ahead-1)
    h = c(sigmafilter[1:(N+i-1)], rep(0, n.ahead))
    r = lapply(realtmp, function(x) c(as.numeric(x[1:(N+i-1),1]), rep(0, n.ahead)))
    epsx = c(resfilter[1:(N+i-1)], rep(0, n.ahead))
    x = c(data[1:(N+i-1)], rep(0, n.ahead))
    z = c(zfilter[1:(N+i-1)], rep(0, n.ahead))
    # forecast of externals is provided outside the system
    mxfi = mxf[1:(N+i-1+n.ahead), , drop = FALSE]
    vxfi = vxf[1:(N+i-1+n.ahead), , drop = FALSE]
    ans = .nreal2garchforecast(ipars, modelinc, idx, mu, mxfi, omega, vxfi, h, realized = r, epsx, z, data = x,
                              N = np, n.ahead, n.sim, returnD = returnDistribution)
    sigmaFor[,i] = ans$h
    seriesFor[,i] = ans$x
    realizedRSVpFor[,i] = ans$r$RSVp
    realizedRSVmFor[,i] = ans$r$RSVm
    if(returnDistribution){
      realizedDpF[,,i] = ans$rforcdist$RSVp
      realizedDmF[,,i] = ans$rforcdist$RSVm
      sigmaDF[,,i] = ans$hforcdist
    }
  }
  fcst = list()
  ...
  fcst$n.roll = n.roll
  fcst$sigmaFor = sigmaFor
  fcst$realizedFor = list(RSVp = realizedRSVpFor, RSVm = realizedRSVmFor)
  fcst$seriesFor = seriesFor
  if(returnDistribution){
    fcst$sigmaDF = sigmaDF
    fcst$realizedDF = list(RSVp = realizedDpF, RSVm = realizedDmF)
  } else{
    fcst$sigmaDF = NULL
    fcst$realizedDF = NULL
  }
...
.nreal2garchforecast = function(ipars, modelinc, idx, mu, mxfi, omega, vxfi, h, realized, epsx, z,
                               data, N, n.ahead, n.sim = 1000, returnD = FALSE)
{
  p = modelinc[9]
  q = modelinc[23]
  beta = ipars[idx["beta",1]:idx["beta",2],1]
  alphap = ipars[idx["alphap",1]:idx["alphap",2],1]
  alpham = ipars[idx["alpham",1]:idx["alpham",2],1]
  xip = ipars[idx["xip",1]:idx["xip",2],1]
  xim = ipars[idx["xim",1]:idx["xim",2],1]
  deltap = ipars[idx["deltap",1]:idx["deltap",2],1]
  deltam = ipars[idx["deltam",1]:idx["deltam",2],1]
  lambdap = ipars[idx["lambdap",1]:idx["lambdap",2],1]
  lambdam = ipars[idx["lambdam",1]:idx["lambdam",2],1]
  lambdapm = ipars[idx["lambdapm",1]:idx["lambdapm",2],1]
  tau1p = ipars[idx["eta1p",1]:idx["eta1p",2],1]
  tau2p = ipars[idx["eta2p",1]:idx["eta2p",2],1]
  tau1m = ipars[idx["eta1m",1]:idx["eta1m",2],1]
  tau2m = ipars[idx["eta2m",1]:idx["eta2m",2],1]
  dist = c("norm", "snorm", "std", "sstd","ged", "sged", "nig", "ghyp", "jsu", "ghst")[modelinc[21]]
  if(modelinc[15]>0){
    omega = omega + vxfi%*%t(matrix(ipars[idx["vxreg",1]:idx["vxreg",2],1], ncol = modelinc[15]))
  }
  ...
  zs = lapply(1:n.ahead, function(i) rdist(dist, n = n.sim, mu = 0, sigma = 1, skew = ipars[idx["skew",1],1], shape = ipars[idx["shape",1],1],
                                           lambda = ipars[idx["ghlambda",1],1]))
  u = lapply(1:n.ahead, function(i) rmvnorm(n.sim, c(0,0), matrix(c(lambdap,lambdapm,lampdapm,lambdam),nrow = 2)))
  Y = c(rev(tail(h[1:N]^2,p)), rev(tail(realized$RSVp[1:N],q)),rev(tail(realized$RSVm[1:N],q)))
  Y = log(Y)
  A = cbind( rbind(matrix(beta, nrow=1), if(p>1) cbind(diag(p-1), matrix(0, nrow=p-1,ncol=1)) else NULL,
                  matrix(deltap*beta, nrow=1),
                  if(q>1 && p>0) matrix(0, nrow=q-1, ncol=p) else NULL,
                  matrix(deltam*beta, nrow=1),
                  if(q>1 && p>0) matrix(0, nrow=q-1, ncol=p) else NULL),
            rbind(matrix(alphap, nrow=1), if(p>1 && q>0) matrix(0, nrow=p-1,ncol=q) else NULL,
                  matrix(deltap*alphap, nrow=1),
                  if(q>1) cbind(diag(q-1), matrix(0, nrow=q-1,ncol=1)) else NULL,
                  matrix(deltam*alphap, nrow=1),
                  if(q>1) matrix(0, nrow=q-1,ncol=q) else NULL),
            rbind(matrix(alpham, nrow=1), if(p>1 && q>0) matrix(0, nrow=p-1,ncol=q) else NULL,
                  matrix(deltap*alpham, nrow=1),
                  if(q>1) matrix(0, nrow=q-1,ncol=q) else NULL,
                  matrix(deltam*alpham, nrow=1),
                  if(q>1) cbind(diag(q-1), matrix(0, nrow=q-1,ncol=1)) else NULL)

            )
  #tohle je spatne
  e = lapply(1:n.ahead, function(i) rbind(if(p>0) matrix(0, nrow=p, ncol=n.sim) else NULL,
                                          matrix(.taufn(zs[[i]], tau1p, tau2p)+u[[i]][1], nrow=1, ncol=n.sim),
                                          if(q>1) matrix(0, nrow=q-1, ncol=n.sim) else NULL,
                                          matrix(.taufn(zs[[i]], tau1m, tau2m)+u[[i]][2], nrow=1, ncol=n.sim),
                                          if(q>1) matrix(0, nrow=q-1, ncol=n.sim) else NULL))

  sigmaDFor = matrix(NA, nrow = n.ahead, ncol = n.sim)
  realizedDpFor = matrix(NA, nrow = n.ahead, ncol = n.sim)
  realizedDmFor = matrix(NA, nrow = n.ahead, ncol = n.sim)
  xDFor = matrix(NA, ncol = n.ahead, nrow = n.sim)
  hFor = realizedRSVpFor = realizedRSVmFor= xFor = rep(0, n.ahead)
  n = ncol(e[[1]])
  D = matrix(0, ncol = n, nrow=nrow(e[[1]]))

  for(i in 1:n.ahead){
    # we can absorb vxfi into omega and make sure to adjust b at every iteration
    b = c(omega[N+i], if(p>1) rep(0, p-1) else NULL, xip+deltap*omega[N+i], if(q>1) rep(0, q-1) else NULL,xim+deltam*omega[N+i], if(q>1) rep(0, q-1) else NULL)
    D = epsfn(A, e[[i]], b, i, D)
    tmp = matrix(A%^%i%*%Y, nrow = length(Y), ncol = ncol(D)) + D
    sigmaDFor[i,] = sqrt(exp(tmp[1,]))
    realizedDpFor[i,] = exp(tmp[p+1,])
    realizedDmFor[i,] = exp(tmp[p+q+1,])
    h[N+i] = sqrt(exp(mean(tmp[1,])))
    realized$RSVp[N+i] = exp(mean(tmp[p+1,]))
    realized$RSVm[N+i] = exp(mean(tmp[p+q+1,]))
  }
  if(modelinc[4]>0){
    res = arfimaf(ipars, modelinc[1:21], idx, mu, mxfi, h, epsx, z, data, N, n.ahead)
  } else{
  ...
  sol = list(h = h[(N+1):(N+n.ahead)], x = res[(N+1):(N+n.ahead)], r = list(RSVp = realized$RSVp[(N+1):(N+n.ahead)],RSVm = realized$RSVm[(N+1):(N+n.ahead)]),
             hforcdist = if(returnD) sigmaDFor else NULL,
             rforcdist = if(returnD) list(RSVp = realizedDpFor, RSVm =realizedDmFor) else NULL)
  return(sol)
}
...
.real2garchforecast2 = function(fitORspec, data = NULL, n.ahead = 10, n.roll = 0, out.sample = 0,
                               external.forecasts = list(mregfor = NULL, vregfor = NULL), realizedVol = NULL,
                               n.sim = 1000, returnDistribution = TRUE, ...)
{
  # first we filter the data to get the results:
  ...
  fspec = myugarchspec(variance.model = list(model = "real2GARCH",
                     ...
                     distribution.model = model$modeldesc$distribution, fixed.pars = as.list(pars))
  tmp =  xts(data[1:(N + fcreq)], index[1:(N + fcreq)])
  realtmp = list(RSVp = xts(realized$RSVp[1:(N + fcreq)], index[1:(N + fcreq)]),RSVm = xts(realized$RSVm[1:(N + fcreq)], index[1:(N + fcreq)]))
  flt = .real2garchfilter(data = tmp, spec = fspec, n.old = N, realizedVol = realtmp)
  sigmafilter = flt@filter$sigma
  resfilter = flt@filter$residuals
  zfilter = flt@filter$z
...
.real2garchsim = function(fit, n.sim = 1000, n.start = 0, m.sim = 1, startMethod =
                           c("unconditional","sample"), presigma = NA, prereturns = NA,
                         preresiduals = NA, rseed = NA, custom.dist = list(name = NA, distfit = NA),
                         mexsimdata = NULL, vexsimdata = NULL, prerealized = NA, ...)
{
  ...
  startMethod = startMethod[1]
  data = fit@model$modeldata$data
  realized = list(RSVp = coredata(fit@model$modeldata$realizedVol$RSVp)[,1], RSVm = coredata(fit@model$modeldata$realizedVol$RSVm)[,1] )
  N = length(as.numeric(data))
  data = data[1:(N - fit@model$n.start)]
  realized = list(RSVp = realized$RSVp[1:(N - fit@model$n.start)], RSVm = realized$RSVm[1:(N - fit@model$n.start)])
  N = length(as.numeric(data))
  m = fit@model$maxOrder
  resids = fit@fit$residuals
  sigma = fit@fit$sigma
  ...
  set.seed(sseed[1])
  usim = rmvnorm(m.sim*n, 0, matrix(c(ipars[idx["lambdap",1]],ipars[idx["lambdapm",1]],ipars[idx["lambdapm",1]],ipars[idx["lambdam",1]]),nrow = 2))
  upsim = rbind(matrix(tail(up,m), nrow=m, ncol=m.sim), matrix(usim[,1], nrow=n, ncol=m.sim))
  umsim = rbind(matrix(tail(um,m), nrow=m, ncol=m.sim), matrix(usim[,2], nrow=n, ncol=m.sim))
    if(startMethod == "unconditional"){
    z = rbind(matrix(0, nrow = m, ncol = m.sim), z)
  } else{
    z = rbind(matrix(tail(fit@fit$z, m), nrow = m, ncol = m.sim), z)
  ...
  #tady bude potreba intervence
  if(!is.na(prerealized[1])){
    prerealized = lapply(prerealized,as.vector)
    if(length(prerealized)<m) stop(paste("\nugarchsim-->error: prerealized must be of length ", m, sep=""))
    prereal = matrix(prerealized, nrow = m)
  }
  if(is.na(presigma[1])){
  ...
  if(is.na(prerealized[1])){
    if(startMethod[1] == "unconditional"){
      xEst = exp(.uncreal2garchr(ipars, idx))
      prereal = lapply(xEst, function(x,m) as.numeric(rep(x, m)),m = m)}
    else{
      prereal  = lapply(realized, tail, m)
    }
  }
  ...
  h = c(presigma^2, rep(0, n))
  x = c(prereturns, rep(0, n))
  r = lapply(prereal, function(x,n) c(x, rep(0, n)),n = n)
  constm = matrix(ipars[idx["mu",1]:idx["mu",2], 1], ncol = m.sim, nrow = n + m)

  # outpus matrices
  sigmaSim =  matrix(0, ncol = m.sim, nrow = n.sim)
  realizedSim =  lapply(realized, function(x) matrix(0, ncol = m.sim, nrow = n.sim))
  tauSim = lapply(realized, function(x) matrix(0, ncol = m.sim, nrow = n.sim))
  seriesSim = matrix(0, ncol = m.sim, nrow = n.sim)
  residSim =  matrix(0, ncol = m.sim, nrow = n.sim)
  z[is.na(z) | is.nan(z) | !is.finite(z)] = 0
  ...
  for(i in 1:m.sim){
    ...
    res = c(preres, rep(0, n))
    ans1 = try(.C("real2garchsimC", model = as.integer(modelinc[1:35]),
                  pars = as.double(ipars[,1]),
                  idx = as.integer(idx[,1]-1),
                  res = as.double(res),
                  vexdata = as.double(vexsim[[i]]),
                  ...
                  taup = double(n+m),
                  taum = double(n+m),
                  rsvp = as.double(r$RSVp),
                  rsvm = as.double(r$RSVm),
                  up = as.double(upsim[,i]),
                  um = as.double(umsim[,i]),
                  PACKAGE = "rugarch"), silent = TRUE)
    ...
    sigmaSim[,i] = ans1$h[(n.start + m + 1):(n+m)]^(1/2)
    residSim[,i] = ans1$res[(n.start + m + 1):(n+m)]
    realizedpSim[,i] = ans1$rsvp[(n.start + m + 1):(n+m)]
    realizedmSim[,i] = ans1$rsvm[(n.start + m + 1):(n+m)]
    taupSim[,i] = ans1$taup[(n.start + m + 1):(n+m)]
    taumSim[,i] = ans1$taum[(n.start + m + 1):(n+m)]
    # ToDo: change to accomodate modelinc[20]
    if(modelinc[6]>0){
      mxreg = matrix( ipars[idx["mxreg",1]:idx["mxreg",2], 1], ncol = modelinc[6] )
      if(modelinc[20]==0){
  ...
  sim = list(sigmaSim = sigmaSim, seriesSim = seriesSim, residSim = residSim, realizedpSim = realizedpSim, realizedmSim = realizedmSim, upSim = upsim,umSim = umsim,
             taupSim = taupSim, taumSim = taumSim)
  model$modeldata$sigma = sigma
  sol = new("uGARCHsim",
            simulation = sim,
            model = model,
