...
.real2garchcon = function(pars, arglist)
{
  ipars = arglist$ipars
  estidx = arglist$estidx
  idx = arglist$model$pidx
  ipars[estidx, 1] = pars
  modelinc = arglist$model$modelinc
  con = sum(ipars[idx["beta", 1]:idx["beta", 2], 1]) + ipars[idx["deltap", 2], 1]*sum(ipars[idx["alphap", 1]:idx["alphap", 2], 1])+ ipars[idx["deltam", 2], 1]*sum(ipars[idx["alpham", 1]:idx["alpham", 2], 1])
  return(con)
}
