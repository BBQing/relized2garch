...
.garchstart = function(pars, arglist)
{
	...
	ans = switch(arglist$model$modeldesc$vmodel,
			...
			real2GARCH = .real2garchstart(pars,arglist))
...
.real2garchstart = function(pars, arglist)
{
  data = arglist$data
  realized = arglist$realized
  model = arglist$model
  ...
  start.names = names(start.pars)
  pars = .meqstart(pars, arglist)
###c 31.1. tato cast je problematicka
  tmp = cbind(log(realized[1:length(data),]), log(data^2)) ###d Zde by se dalo zmenit podle implementace, nicmene je potreba predelat pro
  ###d 31.1. [n,] opravuje problem s colnames - nicmene se nemazou NA's ve sloupcich
  ###1.2. zmena na merge.xts - vraceno zpet na cbind
  tmp[!is.finite(tmp[,1]),1] = NA
  tmp[!is.finite(tmp[,2]),2] = NA
  tmp[!is.finite(tmp[,3]),3] = NA
  tmp = na.omit(tmp)
 # cat(dim(tmp),head(tmp,2), class(tmp),sep = '\n',file = "pnames.txt",append = FALSE) ###c tady hapruje colnames assignment
  colnames(tmp)<-c("RSVp","RSVm","Sqr")
 # cat(tmp[!is.finite(tmp[,3]),3],sep = '\n',file = "pnames.txt",append = TRUE) ###c test NA
  modp = lm(RSVp~Sqr, data = as.data.frame(tmp))
  modm = lm(RSVm~Sqr, data = as.data.frame(tmp))


  xipstart = coef(modp)[1]
  ximstart = coef(modm)[1]
  lambdapstart = sd(modp$residuals)/2
  lambdamstart = sd(modm$residuals)/2
  lambdapmstart = cov(modp$residuals,modm$residuals)/2
  ...
  ### Tohle se musi predelat na 23 a 24
  if(modelinc[8]>0){
    gpnames = paste("alpha",1:modelinc[8],sep="")
    pxd = which(is.na(pars[idx["alpha", 1]:idx["alpha", 2], 5]))
    if(length(pxd)>0) pars[(idx["alpha", 1]:idx["alpha", 2])[pxd], 5] = 0
  ...
  if(modelinc[23]>0){
    gpnames = paste("alphap",1:modelinc[23],sep="")
    pxd = which(is.na(pars[idx["alphap", 1]:idx["alphap", 2], 5]))
    if(length(pxd)>0) pars[(idx["alphap", 1]:idx["alphap", 2])[pxd], 5] = 0
    pxd = which(is.na(pars[idx["alphap", 1]:idx["alphap", 2], 6]))
    if(length(pxd)>0) pars[(idx["alphap", 1]:idx["alphap", 2])[pxd], 6] =  1
    pars[idx["alphap", 1]:idx["alphap", 2], 1] = rep(0.05/modelinc[23], modelinc[23])
    sp = na.omit(match(start.names, gpnames))
    if(length(sp)>0){
      for(i in 1:length(sp)) pars[gpnames[sp[i]], 1] = as.numeric(start.pars[gpnames[sp[i]]])
    }
  ...
  if(modelinc[24]>0){
    gpnames = paste("alpham",1:modelinc[24],sep="")
    pxd = which(is.na(pars[idx["alpham", 1]:idx["alpham", 2], 5]))
    if(length(pxd)>0) pars[(idx["alpham", 1]:idx["alpham", 2])[pxd], 5] = 0
    pxd = which(is.na(pars[idx["alpham", 1]:idx["alpham", 2], 6]))
    if(length(pxd)>0) pars[(idx["alpham", 1]:idx["alpham", 2])[pxd], 6] =  1
    pars[idx["alpham", 1]:idx["alpham", 2], 1] = rep(0.05/modelinc[24], modelinc[24])
    sp = na.omit(match(start.names, gpnames))
    if(length(sp)>0){
      for(i in 1:length(sp)) pars[gpnames[sp[i]], 1] = as.numeric(start.pars[gpnames[sp[i]]])
    }
  ...
  if(modelinc[29] > 0){
    gqnames = "eta1p1"
    if(is.na(pars[idx["eta1p", 1]:idx["eta1p", 2], 5])) pars[idx["eta1p", 1]:idx["eta1p", 2], 5] = -0.5
    if(is.na(pars[idx["eta1p", 1]:idx["eta1p", 2], 6])) pars[idx["eta1p", 1]:idx["eta1p", 2], 6] = 0.5
    pars[idx["eta1p", 1]:idx["eta1p", 2], 1] = 0.1
    if(any(substr(start.names, 1, 4) == "eta1p")){
      j = which(substr(start.names, 1, 4) == "eta1p")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 4) == "eta1p")){
      j = which(substr(fixed.names, 1, 4) == "eta1p")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[31] > 0){
    gqnames = "eta1m1"
    if(is.na(pars[idx["eta1m", 1]:idx["eta1m", 2], 5])) pars[idx["eta1m", 1]:idx["eta1m", 2], 5] = -0.5
    if(is.na(pars[idx["eta1m", 1]:idx["eta1m", 2], 6])) pars[idx["eta1m", 1]:idx["eta1m", 2], 6] = 0.5
    pars[idx["eta1m", 1]:idx["eta1m", 2], 1] = 0.1
    if(any(substr(start.names, 1, 4) == "eta1m")){
      j = which(substr(start.names, 1, 4) == "eta1m")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 4) == "eta1m")){
      j = which(substr(fixed.names, 1, 4) == "eta1m")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[30] > 0){
    gqnames = "eta2p1"
    if(is.na(pars[idx["eta2p", 1]:idx["eta2p", 2], 5])) pars[idx["eta2p", 1]:idx["eta2p", 2], 5] = -0.5
    if(is.na(pars[idx["eta2p", 1]:idx["eta2p", 2], 6])) pars[idx["eta2p", 1]:idx["eta2p", 2], 6] = 0.5
    pars[idx["eta2p", 1]:idx["eta2p", 2], 1] = 0.05
    if(any(substr(start.names, 1, 4) == "eta2p")){
      j = which(substr(start.names, 1, 4) == "eta2p")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 4) == "eta2p")){
      j = which(substr(fixed.names, 1, 4) == "eta2p")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[32] > 0){
    gqnames = "eta2m1"
    if(is.na(pars[idx["eta2m", 1]:idx["eta2m", 2], 5])) pars[idx["eta2m", 1]:idx["eta2m", 2], 5] = -0.5
    if(is.na(pars[idx["eta2m", 1]:idx["eta2m", 2], 6])) pars[idx["eta2m", 1]:idx["eta2m", 2], 6] = 0.5
    pars[idx["eta2m", 1]:idx["eta2m", 2], 1] = 0.05
    if(any(substr(start.names, 1, 4) == "eta2m")){
      j = which(substr(start.names, 1, 4) == "eta2m")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 4) == "eta2m")){
      j = which(substr(fixed.names, 1, 4) == "eta2m")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[27] > 0){
    gqnames = "deltap"
    if(is.na(pars[idx["deltap", 1]:idx["deltap", 2], 5])) pars[idx["deltap", 1]:idx["deltap", 2], 5] = 0.1
    if(is.na(pars[idx["deltap", 1]:idx["deltap", 2], 6])) pars[idx["deltap", 1]:idx["deltap", 2], 6] = 5
    pars[idx["deltap", 1]:idx["deltap", 2], 1] = 1
    if(any(substr(start.names, 1, 5) == "deltap")){
      j = which(substr(start.names, 1, 5) == "deltap")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 5) == "deltap")){
      j = which(substr(fixed.names, 1, 5) == "deltap")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[28] > 0){
    gqnames = "deltam"
    if(is.na(pars[idx["deltam", 1]:idx["deltam", 2], 5])) pars[idx["deltam", 1]:idx["deltam", 2], 5] = 0.1
    if(is.na(pars[idx["deltam", 1]:idx["deltam", 2], 6])) pars[idx["deltam", 1]:idx["deltam", 2], 6] = 5
    pars[idx["deltam", 1]:idx["deltam", 2], 1] = 1
    if(any(substr(start.names, 1, 5) == "deltam")){
      j = which(substr(start.names, 1, 5) == "deltam")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 5) == "deltam")){
      j = which(substr(fixed.names, 1, 5) == "deltam")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[33] > 0){
    gqnames = "lambdap"
    if(is.na(pars[idx["lambdap", 1]:idx["lambdap", 2], 5])) pars[idx["lambdap", 1]:idx["lambdap", 2], 5] = TinY
    if(is.na(pars[idx["lambdap", 1]:idx["lambdap", 2], 6])) pars[idx["lambdap", 1]:idx["lambdap", 2], 6] = 5
    pars[idx["lambdap", 1]:idx["lambdap", 2], 1] = lambdapstart
    if(any(substr(start.names, 1, 6) == "lambdap")){
      j = which(substr(start.names, 1, 6) == "lambdap")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 6) == "lambdap")){
      j = which(substr(fixed.names, 1, 6) == "lambdap")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[34] > 0){
    gqnames = "lambdam"
    if(is.na(pars[idx["lambdam", 1]:idx["lambdam", 2], 5])) pars[idx["lambdam", 1]:idx["lambdam", 2], 5] = TinY
    if(is.na(pars[idx["lambdam", 1]:idx["lambdam", 2], 6])) pars[idx["lambdam", 1]:idx["lambdam", 2], 6] = 5
    pars[idx["lambdam", 1]:idx["lambdam", 2], 1] = lambdamstart
    if(any(substr(start.names, 1, 6) == "lambdam")){
      j = which(substr(start.names, 1, 6) == "lambdam")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 6) == "lambdam")){
      j = which(substr(fixed.names, 1, 6) == "lambdam")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[35] > 0){
    gqnames = "lambdapm"
    if(is.na(pars[idx["lambdapm", 1]:idx["lambdapm", 2], 5])) pars[idx["lambdapm", 1]:idx["lambdapm", 2], 5] = -5
    if(is.na(pars[idx["lambdapm", 1]:idx["lambdapm", 2], 6])) pars[idx["lambdapm", 1]:idx["lambdapm", 2], 6] = 5
    pars[idx["lambdapm", 1]:idx["lambdapm", 2], 1] = lambdapmstart
    if(any(substr(start.names, 1, 6) == "lambdapm")){
      j = which(substr(start.names, 1, 6) == "lambdapm")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 6) == "lambdapm")){
      j = which(substr(fixed.names, 1, 6) == "lambdapm")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[25] > 0){
    gqnames = "xip"
    if(is.na(pars[idx["xip", 1]:idx["xip", 2], 5])) pars[idx["xip", 1]:idx["xip", 2], 5] = -10
    if(is.na(pars[idx["xip", 1]:idx["xip", 2], 6])) pars[idx["xip", 1]:idx["xip", 2], 6] = 10
    pars[idx["xip", 1]:idx["xip", 2], 1] = xipstart
    if(any(substr(start.names, 1, 2) == "xip")){
      j = which(substr(start.names, 1, 2) == "xip")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 2) == "xip")){
      j = which(substr(fixed.names, 1, 2) == "xip")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
  ...
  if(modelinc[26] > 0){
    gqnames = "xim"
    if(is.na(pars[idx["xim", 1]:idx["xim", 2], 5])) pars[idx["xim", 1]:idx["xim", 2], 5] = -10
    if(is.na(pars[idx["xim", 1]:idx["xim", 2], 6])) pars[idx["xim", 1]:idx["xim", 2], 6] = 10
    pars[idx["xim", 1]:idx["xim", 2], 1] = ximstart
    if(any(substr(start.names, 1, 2) == "xim")){
      j = which(substr(start.names, 1, 2) == "xim")
      gqmatch = charmatch(start.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(start.pars[j])
    }
    if(any(substr(fixed.names, 1, 2) == "xim")){
      j = which(substr(fixed.names, 1, 2) == "xim")
      gqmatch = charmatch(fixed.names[j],gqnames)
      pars[gqnames[gqmatch], 1] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 5] = as.numeric(fixed.pars[j])
      pars[gqnames[gqmatch], 6] = as.numeric(fixed.pars[j])
